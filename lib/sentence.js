...
Sentence.isSatisfiable = function(array) {
  const models = this.generateModels(array);
  for (let i = 0; i < models.length; i++) {
    if (this.evaluate(array, models[i])) {
      return true;
    }
  }
  return false;
...
Sentence.generateModels = function(array) {
  const atomics = this.atomics(array);
  const subsets = this.subsets(atomics);
...
Sentence.subsets = function(array) {
  ...
  const subs = this.subsets(array.slice(0, array.length - 1));
